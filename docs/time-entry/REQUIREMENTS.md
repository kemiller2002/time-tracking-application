...
| OQ-6 | Who is the actor on a change made in the browser? | TE-R-031, TE-R-052 | **RESOLVED by DF-TE-0016** — a person signs in with Google or Apple, and the actor is `google:<sub>` or `apple:<sub>`. A command carrying no identity is refused rather than attributed to the former `browser` placeholder. Attribution is *claimed*, not cryptographically verified: signatures are not checked, and the verifiable authority for a change is the commit GitHub authenticates. Making the recorded actor independently verifiable is WI-0054. |
...
| TE-R-124 | A person MUST sign in with Google or Apple; the actor recorded MUST be that identity's provider and subject; a command with no identity MUST be refused rather than attributed. | user instruction (OQ-6); `DF-TE-0016` |

...
| OQ-9 | What is the monthly tracking target, and who sets it? | TE-R-082 | `static-ui-screens/month.html` renders a progress bar against "80h target" and states "Formal program determination: Not evaluated". No repository document says where 80 hours comes from, whether it varies by person or month, or what a determination would be. The month view therefore reports the figures **without** a target or a progress bar: rendering a bar against an invented number would be inventing a requirement. |
| OQ-10 | Is overlapping recorded time a defect, and against what? | TE-R-083 | `static-ui-screens/review.html` lists "No overlapping time" as a daily check. The domain models an entry's **duration**, not its interval — there is no start or end on an entry — so overlap is not computable from what is stored. Whether entries may overlap at all, and whether that is checked against clock times the ledger does not currently keep, is unstated. The review screen therefore omits the check: a tick beside it would be an assurance nothing checked. |
| OQ-11 | May two split children claim the same piece of evidence? | TE-R-045 | A split reassigns the source's evidence to its children. Whether the same document may support two of them is unstated: it is plausible that one brief covers both halves of a session, and equally plausible that reassignment is meant to partition. The transition requires each claimed item to exist on the source but does **not** forbid two children claiming it, because forbidding it would be inventing the stricter reading. |

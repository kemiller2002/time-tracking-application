...
export class MemoryLedgerStore {
  ...
  mergeActivities(input,actor='owner'){
    ...
    const sources=ids.map((id,index)=>this.assertVersion(id,input.base_versions?.[index]));sources.forEach(s=>this.#assertRecorded(s));const startedAt=new Date(Math.min(...sources.map(x=>Date.parse(x.started_at))));const endedAt=new Date(Math.max(...sources.map(x=>Date.parse(x.ended_at))));
